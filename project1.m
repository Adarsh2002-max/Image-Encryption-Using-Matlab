...
imshow(x);
title('Secret Image');
[r c g]=size(a);    % resizing 
x=imresize(x,[r c]);   % making carrier image & secret image size equal
ra=a(:,:,1);
ga=a(:,:,2);
ba=a(:,:,3);
rx=x(:,:,1);
...
for i=1:r
    for j=1:c
       nc(i,j)= bitand(ra(i,j),254); %10110101 by 11111110 (254) 
       ns(i,j)= bitand(rx(i,j),128);  %1000000 (128) 
       ds(i,j)=ns(i,j)/128;
